...
// WindowsStorage implements StorageProvider for Windows using BadgerDB
type WindowsStorage struct {
	db *badger.DB
}

// NewWindowsStorage creates a new Windows storage provider
func NewWindowsStorage() *WindowsStorage {
	return &WindowsStorage{}
}

// GetDefaultStoragePath returns the default storage path for Windows (%APPDATA%/Heimdal/db)
func GetDefaultStoragePath() (string, error) {
	...
	dbPath := filepath.Join(appData, "Heimdal", "db")
	return dbPath, nil
}

// Open initializes the storage backend
func (w *WindowsStorage) Open(path string, options *platform.StorageOptions) error {
...
func (w *WindowsStorage) Close() error {
	if w.db == nil {
		return nil
	}
...
func (w *WindowsStorage) Get(key string) ([]byte, error) {
	if w.db == nil {
...
func (w *WindowsStorage) Set(key string, value []byte) error {
	if w.db == nil {
...
func (w *WindowsStorage) Delete(key string) error {
	if w.db == nil {
		return fmt.Errorf("storage not initialized")
	}
...
func (w *WindowsStorage) List(prefix string) ([]string, error) {
	if w.db == nil {
		return nil, fmt.Errorf("storage not initialized")
	}
...
func (w *WindowsStorage) Batch(ops []platform.BatchOp) error {
	if w.db == nil {
		return fmt.Errorf("storage not initialized")
	}
...
func (w *WindowsStorage) RunGarbageCollection() error {
	if w.db == nil {
		return fmt.Errorf("storage not initialized")
	}
...
func (w *WindowsStorage) GetDatabaseSize() (int64, error) {
	if w.db == nil {
